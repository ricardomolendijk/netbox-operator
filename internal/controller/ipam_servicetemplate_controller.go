...
// Neither create nor delete on netboxservicetemplates: the operator reads CRs and writes their status and
...
// NetBoxServiceTemplate's controller is one line, the same one NetBoxTag's is. ipam.Service minus its parent, with a database-backed identity where the other has a convention (internal/registry/ipam_servicetemplate.go).
