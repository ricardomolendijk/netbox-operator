...
// Neither create nor delete on netboxfhrpgroupassignments: the operator reads CRs and writes their status and
...
// NetBoxFHRPGroupAssignment's controller is one line, the same one NetBoxTag's is. A bare ChangeLoggedModel: no tags, no custom fields, no provenance stamp, a generic-FK pair in its natural key and a CASCADE-ed containment parent -- still one line (internal/registry/ipam_fhrpgroupassignment.go).
