...
// NetBoxASNRange's controller is one line, the same one NetBoxTag's is. A span of ASNs with a PROTECT-ed tenant, which is the reference that makes a NetBoxTenant refuse to delete (internal/registry/ipam_asnrange.go).
