...
// NetBoxRIR's controller is one line, the same one NetBoxTag's is. An OrganizationalModel
// with one column of its own, the root of the allocation registry three other kinds require
// -- all of it data on its Descriptor (internal/registry/ipam_rir.go). This file exists to
// name the kind and to carry its RBAC.
