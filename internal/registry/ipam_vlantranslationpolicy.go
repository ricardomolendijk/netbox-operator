...
func ipamVLANTranslationPolicyDescriptor() Descriptor {
	return Descriptor{
		...
		// Deletes, like every kind since #304 retired the per-kind Retain default -- and it is
		// the ipam.VLANGroup reading rather than an oversight (#176, #186). The rule decision
		// #176 turns on is whether deletion destroys *state*: a translation policy is a table
		// of rewrites, recreated verbatim from the manifest, and deleting one frees no address,
		// no VLAN ID and no range. It belongs with the configuration kinds, and it has to: the
		// ticket's own acceptance criterion is that deleting the policy CR leaves nothing
		// behind in NetBox, which retaining it would make impossible. No DataLossOnDelete
		// either -- the one per-kind deletion flag left -- because there is no stored value to
		// strip off anything.
		//
