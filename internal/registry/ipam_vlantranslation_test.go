...
func TestVLANTranslationDescriptorsAreRegisteredAndValid(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.kind, func(t *testing.T) {
			...
			// that deleting the policy CR leaves nothing behind in NetBox, which retaining
			// would make impossible. Since #304 every kind defaults to Delete and
			// DataLossOnDelete is the only per-kind deletion flag left, so that is what there
			// is to assert: neither kind names a delete a finalizer should refuse.
			if d.DataLossOnDelete {
				t.Errorf("DataLossOnDelete = true; a VLAN translation table is configuration, " +
					"not an allocation, and deleting one destroys no data NetBox would not " +
					"recreate from the manifest (#176, #186, docs/concepts/deletion.md)")
			}
		})
	}
}
