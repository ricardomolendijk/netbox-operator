...
| `InvalidConfig` | `Ready=False`, `Authenticated=Unknown`, `VersionSupported=Unknown` | 2m | `spec.url` is empty, unparseable or not `http`/`https`; or the CA bundle Secret exists but is missing its key (default `ca.crt`) or holds no usable certificate |
