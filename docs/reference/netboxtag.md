...
| `provenance` | `object` | the engine | **never, on this kind.** `extras.Tag` inherits django-taggit's `TagBase` and neither `TagsMixin` nor `CustomFieldsMixin` ([`netbox-schema.md` → `extras.Tag`](../netbox-schema.md)), so a tag cannot be tagged and has no `custom_fields`. A `NetBoxTag` is a fully managed object with no provenance stamp, permanently — see [provenance](../operations/provenance.md) for what `NetBoxSweep` is therefore allowed to do with one |
