...
| `kubectl delete` hangs, `Reason=Protected` | `kubectl describe <kind> <name>` | `Deleting=False`, `Reason=Protected` naming the blockers | NetBox refuses a delete while something still references the object. Delete the blockers -- or, when they are CRs in this cluster, annotate `netbox.kubeforge.org/cascade-delete=true` and the operator deletes them for you. See [deletion](../concepts/deletion.md#cascading-a-refused-delete). |
