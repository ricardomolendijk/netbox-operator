...
| [Generic references](reference/genericref.md) | The union shape a polymorphic foreign key takes in a spec: one member per legal target, `<= 1` versus `== 1`, why an empty union clears the reference while an absent one does not, and the five unions that ship: `IPAssignment`, `ScopeRef`, `ContactAssignmentTarget`, `FHRPInterface` and `ServiceParent` |
