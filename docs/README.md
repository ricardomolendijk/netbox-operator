...
| [`NetBoxProvider`](reference/netboxprovider.md) | The root of the `circuits` app, and the third route to the same one-candidate identity: no `meta.constraints` at all, so `slug` alone -- but off the *model's own* column-level unique rather than a base class's, with `name` unique beside it and deliberately not a second candidate, plus the one to-many reference in the block and why a partly-resolved `asns` list is written as nothing rather than as a shorter list |
| [`NetBoxProviderAccount`](reference/netboxprovideraccount.md) | The kind that separates a constraint condition the operator can reproduce from one it cannot: `(provider, account)` is the identity and `(provider, name)` is a real UniqueConstraint that can never be one, because `~Q(name=...)` is not a null pin and NetBox has no filter for it -- the extractor says so too, and `name` is still written, because unusable as an identity is not the same as unmanaged |
| [`NetBoxProviderNetwork`](reference/netboxprovidernetwork.md) | The same generated constraint name as the account's unusable one, with the opposite answer: no `condition=` clause, so `(provider, name)` is reproducible exactly as written -- plus a `name` that is unique per provider and not globally, a `service_id` with no unique at all, and an object nothing in this build points at yet because the termination kind that needs it is deferred |
| [`NetBoxCircuitType`](reference/netboxcircuittype.md) | A model that declares no columns and no constraints of its own, so every field and the identity come from two base classes up -- and the colour that is deliberately **not** defaulted where `NetBoxRackRole`'s is, because `BaseCircuitType.color` has no Django default and inventing one would PATCH it onto every adopted type |
| [`NetBoxCircuit`](reference/netboxcircuit.md) | Two usable constraints and one declared candidate, argued rather than assumed: why `(provider_account, cid)` can only ever fire on a circuit belonging to a *different* provider, why a permanent 409 beats a wrong adoption there when it does not on `NetBoxRackType`, and why the decision is reversible -- plus `termination_a` and `termination_z`, which are real foreign keys the operator never writes because the relationship is authoritative from the termination's side |
