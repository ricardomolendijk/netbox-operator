...
| [Claims](concepts/claims.md) | What "allocates exactly once" means when a POST can lose its answer, how the deterministic allocation identity gets the same address back after a cluster rebuild and exactly when it does not, why an exhausted pool waits on a timer *and* a watch when neither alone is enough, why a claim always retains its NetBox object and how the operator reports what it left behind, and what `AllocationConflict`, `ReclaimedOutsidePool` and `PoolNotAllocatable` each want you to go and fix |
...
| [`NetBoxTenant`](reference/netboxtenant.md) | The kind almost every IPAM object points at: a group that is part of the identity, `group_id__isnull` pinned rather than omitted, why `tenantRef` does not cascade, and what a `PROTECT`-refused delete looks like |
