...
| [`NetBoxRole`](reference/netboxrole.md) | The kind that disambiguates three things called "role": `ipam.Role` here, `dcim.DeviceRole` behind a different alias and a different endpoint, and `NetBoxIPAddress.role` which is a string — plus why `weight` is a pointer and why `SET_NULL` on every referrer means deleting one silently un-roles its referrers rather than being refused |
...
| [`NetBoxModuleTypeProfile`](reference/netboxmoduletypeprofile.md) | One of the two kinds in NetBox's `dcim` catalogue with **no `slug` column** -- `dcim.Manufacturer`, `dcim.DeviceType`, `dcim.RackType` and `dcim.Platform` all have one: no `meta.constraints` either, so the committed IR reports no natural-key candidate at all and `name` is hand-declared off the column-level `UNIQUE` -- the same extractor gap [#274](https://github.com/ricardomolendijk/netbox-operator/issues/274) records for the power kinds, reached from the other direction -- plus a JSON Schema column the operator carries opaquely because NetBox is the validator and a second copy would drift |
| [`NetBoxModuleType`](reference/netboxmoduletype.md) | The one kind in the module block whose key the IR supplies outright, `(manufacturer, model)`, and the one with **no fallback**: `dcim.RackType` and `dcim.DeviceType` both have a `(manufacturer, slug)` candidate to fall to and this model has no `slug` at all, so a rename is a new object rather than a PATCH -- plus the rename that fails silently, `attribute_data` on the model and `attributes` on the serializer, and nine CounterCacheFields NetBox answers 200 for and never sets |
| [`NetBoxModuleBay`](reference/netboxmodulebay.md) | The fourth kind whose identity NetBox does not enforce, and the first to get there with the constraint intact: `(device, module, name)` exists and `module` is nullable, so Postgres does not stop two identically named bays on one chassis and the `(device, name)` variant pins `module_id` to null -- why a bay whose module is pending waits instead of adopting the chassis bay of the same name, why `moduleRef` is the module that *provides* the bay and never the one installed in it, and why the MPTT `parent` has no field and cannot have one |
| [`NetBoxModule`](reference/netboxmodule.md) | The first kind identified by a `OneToOneField` rather than by a constraint or a convention -- `module_bay` is a `ForeignKey` with `unique=True`, so one bay holds one module and that column is the whole key -- plus the one place the *filter* is wider than the column it stands for, `module_bay_id` being a `TreeNodeMultipleChoiceFilter` that also matches descendants, worked through to the three cases it produces and why none of them adopts the wrong object |
