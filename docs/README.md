...
| [`NetBoxPowerPanel`](reference/netboxpowerpanel.md) | The plainest kind in the power block, and the one to read next to `NetBoxRack`: the same required `site` and optional `location`, and the opposite answer to both questions -- a `(site, name)` constraint the database really enforces so there is no null-pinned convention and no ambiguous match, and a `location` that is `PROTECT` here where the rack's is `SET_NULL` |
| [`NetBoxPowerFeed`](reference/netboxpowerfeed.md) | The first kind whose defaults are **not the model's own**: `voltage`, `amperage` and `max_utilization` resolve from the *target NetBox's* configuration, so the CRD carries no default for them, an omitted one reaches neither the create body nor the drift comparison, and a claimed-but-unset pointer is not restored as a zero -- plus four enums that *are* defaulted and why the difference is the whole ticket, why `available_power` is absent rather than promised, and how registering one Descriptor closes a cable-union member declared three tickets ago |
