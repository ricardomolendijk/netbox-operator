...
| [`NetBoxVLANTranslationPolicy`](reference/netboxvlantranslationpolicy.md) | The kind whose identity the schema IR does not carry: `natural_keys: []` because the model declares no `meta.constraints`, so the key is hand-declared from the column-level `UNIQUE` on `name` -- plus the two Kinds it unblocks, why neither it nor its rules carry a provenance stamp when the model mixes in both mixins, and why it is the second kind in `ipam` to default to `deletionPolicy: Delete` |
| [`NetBoxVLANTranslationRule`](reference/netboxvlantranslationrule.md) | The first kind with two natural-key candidates over **different columns**: `(policy, local_vid)` and `(policy, remote_vid)` are both enforced at once, why the second is not a fallback, why a VID swap cannot be applied in one step, and why the operator refuses to pre-validate a constraint NetBox already holds |
