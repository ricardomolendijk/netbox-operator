...
| [`NetBoxWirelessLANGroup`](reference/netboxwirelesslangroup.md) | The third `NestedGroupModel` and the third identity out of one base class: the only one so far with **both** constraint mechanisms, which still lands on `slug` alone -- why `(parent, name)` with no `condition=` clause adds nothing to an identity, why adding the `parent IS NULL` pin every MPTT kind is said to need would make a nested group's slug unfindable, and why this one maps `comments` where the organisational groups do not |
| [`NetBoxWirelessLAN`](reference/netboxwirelesslan.md) | The kind the scope-versus-`site` bug is named for: why there is no `siteRef` and never will be, four natural-key candidates because scope and tenant are *independent* optional terms, an identity no constraint backs so two SSIDs in one scope is a `Conflict` rather than a guess, all four scope members cascading by two different mechanisms that have to be read separately, and why `auth_psk` is deliberately absent rather than inline |
| [`NetBoxWirelessLink`](reference/netboxwirelesslink.md) | The first kind whose identity is a pair of references to one Kind, and the first where that pair is **ordered**: why `(a,b)` and `(b,a)` are two NetBox rows and one physical link, what the reverse natural-key candidate buys and why canonicalising to ascending id would be worse, why neither endpoint can be deferred, a `dcim` status enum on a `wireless` kind, a decimal-as-string distance cleared as `null`, and why having **no** containment parent is a consequence rather than a gap |
| [`NetBoxRackRole`](reference/netboxrackrole.md) | The `dcim.RackRole` half of a name that reads like a nested group and is not one: no `meta.constraints` at all, so `slug` alone off `OrganizationalModel`'s column-level unique -- and `dcim.DeviceRole` minus the self-reference, minus `vmRole`, and minus any need for a `parent IS NULL` variant |
| [`NetBoxRackGroup`](reference/netboxrackgroup.md) | The kind whose name is the whole trap: `dcim.RackGroup` is an `OrganizationalModel` with **no `parent` column**, no `site`, no MPTT tree and no constraints, so there is no nested-group identity to derive, no cycle check to webhook, and a `parent_id` pin would be a filter NetBox's `BaseFilterSet` drops in silence |
| [`NetBoxRackType`](reference/netboxracktype.md) | `dcim.DeviceType`'s identity shape a second time -- `(manufacturer, slug)` then `(manufacturer, model)`, and nothing written until the manufacturer resolves -- plus a `slug` that is *also* globally unique when the constraint does not need it to be, the twelve `RackBase` dimensions shared with the rack through one inline struct, and a `form_factor` the serializer calls optional and Postgres calls NOT NULL |
| [`NetBoxRack`](reference/netboxrack.md) | The physical-plant kind NetBox 4.2's scope change did **not** touch, and the third whose identity NetBox does not enforce: why `siteRef` is a real foreign key rather than a cached column, why both `meta.constraints` being keyed on an *optional* `location` means a location-less rack falls back to a null-pinned `(site, name)` convention and an ambiguous match is a `Conflict`, why `assetTag` is globally unique and still not a candidate, and why a rack gets no owner reference from its site however much the cascade rule looks like it should |
| [`NetBoxRackReservation`](reference/netboxrackreservation.md) | An identity backed by nothing at all -- no constraint, no unique, an ordering that names only `created` -- so `(rack, description)` is a convention and `description` is required for that reason; the only `CASCADE` in the rack hierarchy and so the only owner reference; a `units` array whose order is data rather than a set; and a **required foreign key with no Kind to point at**, which is why `userID` is a raw NetBox primary key and what in the resolver would have to change for it not to be |
| [`NetBoxTunnelGroup`](reference/netboxtunnelgroup.md) | `dcim.RackGroup`'s shape one app over: an `OrganizationalModel` with **no columns of its own**, so no `parentRef`, no tree and no cycle check, `slug` alone from a column-level unique -- plus the counter cache NetBox drops rather than refuses, and why a tunnel takes no owner reference from the group that `PROTECT`s it |
| [`NetBoxTunnel`](reference/netboxtunnel.md) | The kind whose identity depends on a field you may omit: `(group, name)` from the first `UniqueConstraint` and `name` with `?group_id=null` pinned from the conditional one, why declaring `groupRef` decides which applies and an unresolved one makes neither apply, a `status` enum NetBox lets a deployment extend and this CRD cannot read, every foreign key `PROTECT` so there is no containment parent at all -- and why `vpn.TunnelTermination` is deferred rather than inline |
| [`NetBoxIKEProposal`](reference/netboxikeproposal.md) | The plainest kind in `vpn`: four required fields, no reference, no secret, `name` from a column-level unique -- and three closed `ChoiceSet`s with the source lines that prove no deployment can extend them, including a DH group set with holes in it that a `minimum`/`maximum` pair would get wrong |
| [`NetBoxIKEPolicy`](reference/netboxikepolicy.md) | The kind with the secret, and the field that is deliberately **absent**: why there is no `presharedKey` and no `presharedKeySecretRef` either, what the operator does to the column instead (nothing -- never written, never cleared, redacted in logs), why adding a Secret reference later is purely additive, and a to-many `proposals` NetBox requires on create while the CRD leaves it optional so adoption stays possible |
| [`NetBoxIPSecProposal`](reference/netboxipsecproposal.md) | The phase 2 counterpart, and the nullability contrast that explains why one NetBox `ChoiceSet` gets exactly one Go type: the same two algorithm enums as the IKE proposal with the `REQ` taken off, and two independent lifetime columns the operator will not infer from each other |
| [`NetBoxIPSecPolicy`](reference/netboxipsecpolicy.md) | The phase 2 policy, and the one in this app **without** a secret: the same wholesale many-to-many as its IKE twin, plus a PFS group that is a pointer rather than an `EmptyIsNull` string, because an integer has no empty value to write |
| [`NetBoxIPSecProfile`](reference/netboxipsecprofile.md) | Two **required** references on one object and nothing written until both resolve: why the graph converges in any order, why `PROTECT` on both means no containment parent in either direction, and why `(ike_policy, ipsec_policy)` is deliberately not the identity when `name` already is |
| [`NetBoxL2VPN`](reference/netboxl2vpn.md) | `NetBoxVRF`'s two route-target relations a second time with no engine change to go with them, and an identity where **both** candidate columns are unique: why `slug` wins and why there is deliberately no `name` fallback, a `type` the serializer calls optional and Postgres calls `NOT NULL`, a signed identifier with no lower bound, an extensible `status` `ChoiceSet` this CRD cannot read -- and why `vpn.L2VPNTermination` is deferred |
| [Generic references](reference/genericref.md) | The union shape a polymorphic foreign key takes in a spec: one member per legal target, `<= 1` versus `== 1`, why an empty union clears the reference while an absent one does not, and the seven unions that ship: `IPAssignment`, `ScopeRef`, `ContactAssignmentTarget`, `FHRPInterface`, `ServiceParent`, `CableTerminationTarget` and `MACAssignment` |
