...
//
// Like NetBoxPrefix, this kind wants `deletionPolicy: Retain` and cannot default to it: the
// field is declared once on the embedded NetBoxObjectSpec, and redeclaring it here makes
// controller-gen emit a schema the API server rejects. Deleting a range destroys the record of
// who a block of addresses belonged to, so every manifest should say `Retain` explicitly, as
// docs/reference/netboxiprange.md's examples do.
