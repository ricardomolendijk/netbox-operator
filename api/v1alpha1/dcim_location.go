...
type NetBoxLocationSpec struct {
	...
	// which is why `parentRef` is not. The owner reference rule 4 calls for is not written by
	// any code in this build; the declaration is what a kind can state, and the mechanism
	// that reads it is NBO-014's remaining half.
	//
