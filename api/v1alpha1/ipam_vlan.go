...
//
// `deletionPolicy` defaults to `Retain` here, per
// docs/concepts/deletion.md#the-default-depends-on-the-kind: deleting a VLAN destroys its
// change log, its journal entries and every termination hanging off it, and a fresh VLAN with
// the same `vid` is a different object with a different id. The default is data on the kind's
// Descriptor (registry.Descriptor.RetainOnDelete) rather than a CRD marker, because
...
// server rejects outright. `NetBoxVLANGroup` is the neighbour that keeps `Delete`, and that
// contrast is on its own struct.
