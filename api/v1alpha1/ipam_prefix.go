...
//
// `deletionPolicy` still defaults to `Delete` here, and it should not. Deleting a prefix
...
// id. IPAM kinds want `Retain`. It is not implemented because it cannot be, per kind, in this
// API: `deletionPolicy` is declared once on the embedded NetBoxObjectSpec, and redeclaring it
// on this struct makes controller-gen emit `allOf: [{default: Retain}, {default: Delete}]` --
// a schema the API server rejects outright -- while the engine would still read the
// envelope's own copy, which JSON decoding leaves empty. Giving a kind its own default takes
// a change to the shared envelope, which is deliberately out of scope for one kind. Until
// then every NetBoxPrefix manifest should say `deletionPolicy: Retain` explicitly, which is
// what docs/reference/netboxprefix.md's examples do.
