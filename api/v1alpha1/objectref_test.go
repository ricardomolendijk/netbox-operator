...
func TestRefAliasTargets(t *testing.T) {
	for _, tc := range []struct {
		...
		// The third target with no slug column, and the first in dcim: dcim.ModuleTypeProfile
		// is unique on `name` alone (docs/netbox-schema.md), so a slug-mode ref matches
		// nothing there either. The other reference target with no slug column and the reason
		// the module block has two: neither dcim.ModuleTypeProfile nor dcim.ModuleType has one,
		// where dcim.Manufacturer, dcim.DeviceType, dcim.RackType and dcim.Platform all do
